...
export const drawFactionAbilityCard = async (params: CardDrawParams) => {
  ...
  if (card.Type === "Special Victory Condition") {
    ctx.drawImage(imgs[1], 0, 0, ctx.canvas.width, ctx.canvas.height);
  } else if (isFactionAlliance) {
    ctx.drawImage(getFACard(card.Faction), 0, 0, ctx.canvas.width, ctx.canvas.height);
  } else {
    ctx.drawImage(imgs[0], 0, 0, ctx.canvas.width, ctx.canvas.height);
  }
